...
class PDrawStrategy {
   public:
	...
	virtual void draw_point(const std::shared_ptr<SDL_Renderer> &renderer, float x, float y) = 0;
	virtual void draw_line(const std::shared_ptr<SDL_Renderer> &renderer, float x1, float y1, float x2, float y2) = 0;
...
class PDrawCartesianStrategy : public PDrawStrategy {
   public:
	...
	~PDrawCartesianStrategy() override = default;
	void draw_point(const std::shared_ptr<SDL_Renderer> &renderer, float x, float y) override {
		SDL_RenderDrawPointF(renderer.get(), x, y);
	}
	void draw_line(const std::shared_ptr<SDL_Renderer> &renderer, float x1, float y1, float x2, float y2) override {
...
class PDrawIsometricStrategy : public PDrawStrategy {
   public:
	...
	~PDrawIsometricStrategy() override = default;
	void draw_point(const std::shared_ptr<SDL_Renderer> &renderer, float x, float y) override {
